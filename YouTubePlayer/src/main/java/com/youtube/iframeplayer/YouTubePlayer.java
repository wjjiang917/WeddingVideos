...
public class YouTubePlayer extends WebView {
    ...
    /**
     * get current playback quality。可能的返回值包括highres、hd1080、hd720、large、medium 和 small。如果当前没有视频，则该函数会返回 undefined
     */
    protected void getPlaybackQuality() {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
     ...
     */
    protected void getAvailableQualityLevels() {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
     ...
     * setPlaybackQuality
     */
    protected void setPlaybackQuality(final String suggestedQuality) {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
    ...
    public interface YouTubeListener {
        ...
        void onReturnAvailableQualityLevels(int[] playbackQualities);
    ...
    public static class Error {
        ...
        @IntDef({INVALID_PARAMETER_IN_REQUEST, HTML_5_PLAYER, VIDEO_NOT_FOUND, VIDEO_NOT_PLAYABLE_IN_EMBEDDED_PLAYER})
        @Retention(RetentionPolicy.SOURCE)
