...
public class YouTubePlayerBridge {
    ...
    @JavascriptInterface
    public void onReturnAvailableQualityLevels(final String[] qualities) {
        mainThreadHandler.post(new Runnable() {
            @Override
            public void run() {
                if (null == qualities){
                ...
                int[] qualityLevels = new int[qualities.length];
                for (int i = 0; i < qualities.length; i++) {
                    if ("small".equalsIgnoreCase(qualities[i]))
                        qualityLevels[i] = YouTubePlayer.PlaybackQuality.SMALL;
                    else if ("medium".equalsIgnoreCase(qualities[i]))
                        qualityLevels[i] = YouTubePlayer.PlaybackQuality.MEDIUM;
                    else if ("large".equalsIgnoreCase(qualities[i]))
                        qualityLevels[i] = YouTubePlayer.PlaybackQuality.LARGE;
                    else if ("hd720".equalsIgnoreCase(qualities[i]))
                        qualityLevels[i] = YouTubePlayer.PlaybackQuality.HD720;
                    else if ("hd1080".equalsIgnoreCase(qualities[i]))
                        qualityLevels[i] = YouTubePlayer.PlaybackQuality.HD1080;
                    else if ("highres".equalsIgnoreCase(qualities[i]))
                        qualityLevels[i] = YouTubePlayer.PlaybackQuality.HIGH_RES;
                    else if ("default".equalsIgnoreCase(qualities[i]))
                        qualityLevels[i] = YouTubePlayer.PlaybackQuality.DEFAULT;
                }
                for (YouTubePlayer.YouTubeListener listener : youTubePlayer.getListeners())
                    listener.onReturnAvailableQualityLevels(qualityLevels);
            }
        });
    }
            ...
            public void run() {
                for (YouTubePlayer.YouTubeListener listener : youTubePlayer.getListeners()) {
                    if ("2".equalsIgnoreCase(error))
                        listener.onError(YouTubePlayer.Error.INVALID_PARAMETER_IN_REQUEST);
